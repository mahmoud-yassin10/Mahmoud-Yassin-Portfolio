...
2. Create the owner account in Firebase Authentication (email/password).
3. After the account exists, create `admin_users/{uid}` in Firestore using the Firebase Console or Admin SDK. The document can contain `{ "role": "owner" }`; the security rules only use the document's existence.
4. Deploy the Flousy functions and rules from the Flousy repo:
...
The dashboard runs on seeded preview data when Firebase env vars or admin access are not available, so the interface can be reviewed safely before production telemetry is connected.

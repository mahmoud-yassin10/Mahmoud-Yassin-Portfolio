...
const FlousyDashboard = () => {
  ...
  const refresh = useCallback(async () => {
    ...
    catch { setNotice("Live data is not available yet, so this is the preview dataset. Check Firestore indexes and admin access when you connect production."); setData(makeDemoData()); }
    finally { setLoading(false); }
  }, [days, user]);
  ...
  return <div className="dashboard-shell">
    ...
    <main className="dashboard-main"><header className="dashboard-topbar"><button className="dashboard-menu-button" onClick={() => setMobileNav(true)}><Menu size={19} /></button><div className="dashboard-breadcrumb"><span>Flousy</span><b>/</b><strong>{activeView === "overview" ? "Control room" : activeView === "funnel" ? "Funnel & retention" : "Feedback inbox"}</strong></div><div className="dashboard-top-actions"><div className="dashboard-status"><span className="dashboard-live-dot" /> {data.live ? "Live" : "Preview"}</div><button className="dashboard-icon-button" onClick={() => void refresh()} disabled={loading} title="Refresh"><RefreshCw size={16} className={loading ? "animate-spin" : ""} /></button></div></header>
